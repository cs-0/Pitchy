...
class PitchSpec: QuickSpec {
    ...
    func spec() {
        ...
        Self.describe("Pitch.Offsets") {
            Self.describe("#init") {
                Self.it("rearrange offsets based on frequency") {
        ...
        Self.describe("Pitch") {
            Self.describe("#init") {
                Self.it("sets correct values for the specified frequency") {
                    offsets.forEach {
                ...
                Self.it("sets a correct offsets for the specified frequency") {
                    offsets.forEach {
                        let pitch = try! Pitch(frequency: $0.frequency)
